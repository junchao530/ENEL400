...
void generate_quality_arc(void) {
  title_labels[2] = lv_label_create(lv_scr_act());
  lv_label_set_text(title_labels[2], "Quality");
  
  arcs[2] = lv_arc_create(lv_scr_act());
  ...
  value_labels[2] = lv_label_create(lv_scr_act());
...
void update_arc_values(void) {
  ...
  if (temperature_increasing) {
    temperature_value++;
    if (temperature_value >= 100) temperature_increasing = false;
  } else {
    temperature_value--;
    if (temperature_value <= 0) temperature_increasing = true;
  }
